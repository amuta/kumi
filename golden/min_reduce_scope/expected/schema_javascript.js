// Autogenerated by Kumi Codegen for schema KUMI_b2fd1af544dc93588b9b7a709b6c06cf01abcea977dd806b5d354cce6673309d
export class KumiCompiledModule {
  ...
  get(name) {
    switch (name) {
      case 'dept_total': return this._dept_total();
      case 'company_total': return this._company_total();
      case 'big_team': return this._big_team();
      case 'dept_total_masked': return this._dept_total_masked();
      default: throw new Error(`Unknown declaration: ${name}`);
    }
  }

  _dept_total(input = this.input) {
    let out = [];
    let t1 = input["depts"];
  ...
  _company_total(input = this.input) {
    let acc_10 = 0;
    let t11 = input["depts"];
  ...
  _dept_total_masked(input = this.input) {
    let out = [];
    let t30 = input["depts"];
    const t45 = 10;
    const t39 = 0;
