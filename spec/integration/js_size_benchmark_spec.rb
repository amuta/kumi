      ...
      schema do
        input do
          (1..inputs_count).each do |i|
            case i % 4
            when 0
              integer :"field_#{i}", domain: 1..100
            when 1
              string :"name_#{i}"
            when 2
              float :"amount_#{i}", domain: 0.0..1000.0
            when 3
              boolean :"flag_#{i}"
            end
          end
        ...
        (1..traits_count).each do |i|
          ...
          when 0  # integer field
            trait :"trait_#{i}", (input.send("field_#{field_idx}") > 50)
          when 1  # string field
            trait :"trait_#{i}", (input.send("name_#{field_idx}") == "Test Name #{field_idx}")
          when 2  # float field
            trait :"trait_#{i}", (input.send("amount_#{field_idx}") >= 100.0)
          when 3  # boolean field
            trait :"trait_#{i}", (input.send("flag_#{field_idx}") == true)
          end
        end

        (1..values_count).each do |i|
          if i <= cascade_depth
            value :"cascade_value_#{i}" do
