...
RSpec.describe "Element :any Hash Access" do
  describe "element :any as alternative to hash objects" do
    ...
    it "handles nested element :any with complex hash structures" do
        ...
        companies: [
          {
            ...
            employees: [
              { "name" => "Alice", "department" => "Engineering", "salary" => 95_000 },
              { "name" => "Bob", "department" => "Marketing", "salary" => 75_000 }
            ]
          },
          {
            company_name: "DataCorp",
            employees: [
              { "name" => "Charlie", "department" => "Research", "salary" => 105_000 },
              { "name" => "Diana", "department" => "Sales", "salary" => 80_000 }
            ]
          }
        ]
      }
      ...
      expect(runner[:company_names]).to eq(%w[TechCorp DataCorp])
      expect(runner[:employee_names]).to eq([%w[Alice Bob], %w[Charlie Diana]])
      expect(runner[:departments]).to eq([%w[Engineering Marketing], %w[Research Sales]])
      expect(runner[:salaries]).to eq([[95_000, 75_000], [105_000, 80_000]])
      expect(runner[:total_salaries]).to eq([170_000, 185_000])
      expect(runner[:avg_salary_per_company]).to eq([85_000.0, 92_500.0])
    end
    ...
    it "handles deep nesting with element :any and aggregations" do
      ...
      expect(runner[:total_floor_area]).to eq([12_000, 7500])
    ...
    it "works with mathematical operations on hash numeric values" do
      ...
      test_data = {
        transactions: [
          { "amount" => 100.50, "fee" => 2.50, "timestamp" => 1_609_459_200, "type" => "purchase" },
          { "amount" => 75.25, "fee" => 1.75, "timestamp" => 1_609_545_600, "type" => "refund" },
          { "amount" => 200.00, "fee" => 5.00, "timestamp" => 1_609_632_000, "type" => "purchase" },
          { "amount" => 150.75, "fee" => 3.25, "timestamp" => 1_609_718_400, "type" => "transfer" }
        ]
      }
  ...
  describe "comparison with explicit hash objects" do
    it "demonstrates equivalent functionality between element :any and hash objects" do
      ...
      expect(runner_any[:names]).to eq(%w[Alice Bob])
      expect(runner_any[:ages]).to eq([30, 25])
      expect(runner_hash[:names]).to eq(%w[Alice Bob])
