...
RSpec.describe 'RegistryV2 Migration Integration' do
  ...
  describe 'Function Name Normalization' do
    ...
    it 'normalizes function calls to qualified names' do
      ...
      expect(result[:max_price][:rows].map { |r| r[:v] }).to eq([100.0])
      expect(result[:total_price][:rows].map { |r| r[:v] }).to eq([149.5])
    end
  end
    ...
    it 'handles complex trait conditions' do
      schema = Module.new do
        ...
        schema do
          ...
          trait :is_active, input.active
          trait :is_verified, input.verified
  ...
  describe 'Array Broadcasting with RegistryV2' do
    it 'detects vectorized operations correctly' do
      ...
      result = schema.from(test_data)
      expect(result[:subtotals][:rows].map { |r| r[:v] }).to eq([20.0, 45.0])
    ...
    it 'handles reduction operations with proper scope resolution' do
      ...
      expect(result[:player_max_scores][:rows].map { |r| r[:v] }).to eq([120, 110])
    end
  end
  ...
  describe 'Type Checking with RegistryV2' do
    it 'validates function signatures from RegistryV2' do
      ...
      result = schema.from(test_data)
      expect(result[:total][:rows].map { |r| r[:v] }).to eq([40])
      expect(result[:maximum][:rows].map { |r| r[:v] }).to eq([25])
    end
  ...
  describe 'Metadata Pipeline Integration' do
    it 'preserves metadata through all passes' do
      ...
      expect(result[:max_number][:rows].map { |r| r[:v] }).to eq([150])
    end
  end
    ...
    it 'handles deeply nested structures with multiple function types' do
      schema = Module.new do
        ...
        schema do
          ...
          trait :has_high_earner, fn(:any?, input.departments.employees.salary > 80000)
      ...
      # Verify nested array access works
      hours_data = result[:employee_total_hours]
      dept_hours = hours_data[:rows].map { |dept_row| 
        dept_row[:v][:rows].map { |emp_row| emp_row[:v] }
      }
      expect(dept_hours).to eq([[60], [30]])
    end
  end
end