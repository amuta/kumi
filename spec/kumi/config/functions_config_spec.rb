...
RSpec.describe "Functions Configuration" do
    ...
    it "validates fixed-size dimension signatures" do
      ...
      fixed_size_sigs.each do |sig_string|
        ...
        signature.in_shapes.flatten.each do |dim|
          expect(dim.size).to be > 0, "Fixed-size dimension must be positive in: #{sig_string}" if dim.fixed_size?
        end
      end
    end
    ...
    it "validates join policies match signature patterns" do
      functions_config.each do |func|
        func["signature"].each do |sig_string|
          ...
          next unless signature.join_policy
  ...
  describe "ruby kernel availability" do
    it "every function has at least one ruby kernel impl" do
      functions_config.each do |func|
        ruby = func.fetch("kernels", []).select { |k| k["backend"].to_s == "ruby" && k["impl"].to_s != "" }
        expect(ruby).not_to be_empty, "Function #{func['name']} has no Ruby kernel"
      end
    end

    it "for any @policy in signatures, there is a ruby kernel gated for that policy" do
      parser = Kumi::Core::Functions::SignatureParser
      functions_config.each do |func|
        policies = func.fetch("signature", []).map { |s| parser.parse(s).join_policy }.compact.uniq
        next if policies.empty?

        policies.each do |policy|
          has = func.fetch("kernels", []).any? do |k|
            k["backend"].to_s == "ruby" &&
              k["when"].is_a?(Hash) &&
              k["when"]["policy"].to_s == policy.to_s &&
              k["impl"].to_s != ""
          end
          expect(has).to be(true), "Function #{func['name']} declares @#{policy} but lacks a Ruby kernel with when: { policy: #{policy} }"
        end
      end
    end
  end
