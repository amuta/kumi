...
RSpec.describe Kumi::Analyzer::Passes::TypeInferencer do
  ...
  describe "#run" do
    ...
    context "with basic declarations" do
      ...
      let(:field_expr) { Kumi::Syntax::TerminalExpressions::Field.new(:age, nil, Kumi::Types::INT) }
      ...
      before do
        state[:topo_order] = [:literal_val, :field_val, :binding_val]
        state[:definitions] = {
    ...
    context "with function calls" do
      ...
      before do
        state[:topo_order] = [:sum, :is_greater]
        state[:definitions] = {
          sum: add_decl,
    ...
    context "with list expressions" do
      ...
      before do
        state[:topo_order] = [:numbers, :empty]
        state[:definitions] = {
          numbers: list_decl,
    ...
    context "with dependencies between declarations" do
      ...
      before do
        state[:topo_order] = [:base_value, :dependent_value]
        state[:definitions] = {
