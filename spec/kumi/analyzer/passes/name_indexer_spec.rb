  ...
  describe ".run" do
    ...
    context "with unique attribute and trait names" do
      ...
      it "stores each declaration and reports zero errors" do
        ...
        expect(definitions[:price]).to be_a(Kumi::Core::Syntax::ValueDeclaration)
        expect(definitions[:vip]).to be_a(Kumi::Core::Syntax::TraitDeclaration)
      end
    end
    ...
    context "when an attribute and a trait share the same name" do
      ...
      it "registers the duplicate and keeps the last declaration in the map" do
        ...
        expect(errors.size).to eq(1)
        expect(result_state[:declarations][:conflict]).to be_a(Kumi::Core::Syntax::TraitDeclaration)
      end
    end
  end
end
