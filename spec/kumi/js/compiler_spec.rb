  ...
  describe "JavaScript compilation" do
    ...
    describe "#compile" do
      ...
      it "supports different output formats" do
        standalone = Kumi::Js.compile(simple_schema, format: :standalone)
        es6 = Kumi::Js.compile(simple_schema, format: :es6)
        
        expect(standalone).to include("const schema = {")
        expect(es6).to include("export class KumiRunner")
        expect(es6).to include("export default {")
      end
    end
