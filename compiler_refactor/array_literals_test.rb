...
if inline_array_decl
  ...
  if inline_array_decl.expression.respond_to?(:elements)
    puts "Elements: #{inline_array_decl.expression.elements.map(&:inspect)}"
  end
end
...
begin
  ...
  [:tier_values, :float_selection].each do |value_name|
    begin
