...
module StatusEffects
  ...
  schema do
    input do
      array :active_effects, elem: { type: :string }
      integer :poison_turns, domain: 0..10
      integer :blessing_turns, domain: 0..10
    ...
    trait :poisoned, fn(:include?, input.active_effects, "poison")
    ...
    value :status_description do
      on :poisoned, :blessed, :enraged, "💀🌟⚡ Poisoned, Blessed, Enraged"
      on :poisoned, :blessed, "💀🌟 Poisoned but Blessed"
      on :poisoned, :enraged, "💀⚡ Poisoned and Enraged"
      on :blessed, :enraged, "🌟⚡ Blessed and Enraged"
      on :poisoned, "💀 Poisoned"
      on :blessed, "🌟 Blessed"
      on :enraged, "⚡ Enraged"
      on :shielded, "🛡️ Shielded"
      base ""
    end

    value :turn_message do
      on :poisoned, "💀 Poison deals damage!"
      base ""
    end

    value :remaining_turns do
      on :poisoned, input.poison_turns
      on :blessed, input.blessing_turns
      on :enraged, input.rage_turns
      on :shielded, input.shield_turns
      base 0
    end
  end
end
