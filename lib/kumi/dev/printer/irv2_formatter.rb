    ...
    module Printer
      class IRV2Formatter
        ...
        def format_operation(idx, op)
          ...
          attrs = op['attrs'] || {}
          extra_attrs = attrs.reject { |k, _| k == 'op' || k == :op }
        ...
        def format_analysis_details
          ...
          @irv2['analysis']['inputs'].each do |input|
            ...
            if input['key_policy'] != defaults['key_policy']
              output << "      key_policy: #{input['key_policy']}"
            end
            if input['on_missing'] != defaults['on_missing']
              output << "      on_missing: #{input['on_missing']}"
            end
