    ...
    class Validator
      def self.validate_context(context, input_meta)
        ...
        context.each do |field, value|
          ...
          if meta[:type] && meta[:type] != :any
            unless type_matches?(value, meta[:type])
          ...
          if meta[:domain]
            unless Domain::Validator.validate_field(field, value, meta[:domain])
      ...
      def self.infer_type(value)
        ...
        when Hash then { hash: [:mixed, :mixed] }
        else :unknown
        end
      end
