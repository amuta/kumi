...
module Kumi
  ...
  class DualRunner
    ...
    def initialize(schema_class, input_data)
      ...
      @ruby_runner = Core::SchemaInstance.new(
      ...
      @dual_mode_enabled = ENV["KUMI_DUAL_MODE"] == "true" || Thread.current[:kumi_dual_mode]

      # Track schema compilation
      @@metrics[:schemas_compiled] += 1 if @dual_mode_enabled
    end
    ...
    # Enable dual mode for the current thread
    def self.enable_dual_mode!
      Thread.current[:kumi_dual_mode] = true
    end

    # Disable dual mode for the current thread
    def self.disable_dual_mode!
      Thread.current[:kumi_dual_mode] = false
    end

    # Execute block with dual mode enabled
    def self.with_dual_mode(&block)
      old_value = Thread.current[:kumi_dual_mode]
      Thread.current[:kumi_dual_mode] = true
      yield
    ensure
      Thread.current[:kumi_dual_mode] = old_value
    end
  ...
  # Auto-print metrics when dual mode is enabled
  at_exit do
    DualRunner.print_metrics if ENV["KUMI_DUAL_MODE"] == "true" && DualRunner.metrics[:schemas_compiled] > 0
  end
end
