    ...
    module Loader
      ...
      def build_dtype_rule_from_yaml(dtype_spec)
        ...
        when Hash
          ...
          raise Bug, "invalid dtype specification: #{dtype_spec.inspect}"
        end
      end
      ...
      def build_dtype_rule_from_hash(spec)
        ...
        case rule_type
        when "same_as"
          param = spec.fetch("param") { raise Bug, "same_as rule requires 'param' key" }
        ...
        when "promote"
          ...
          param_syms = Array(params).map(&:to_sym)
          Kumi::Core::Functions::TypeRules.build_promote(*param_syms)

        when "element_of"
          param = spec.fetch("param") { raise Bug, "element_of rule requires 'param' key" }
          Kumi::Core::Functions::TypeRules.build_element_of(param.to_sym)

        when "unify"
          param1 = spec.fetch("param1") { raise Bug, "unify rule requires 'param1' key" }
          param2 = spec.fetch("param2") { raise Bug, "unify rule requires 'param2' key" }
          Kumi::Core::Functions::TypeRules.build_unify(param1.to_sym, param2.to_sym)

        when "common_type"
          param = spec.fetch("param") { raise Bug, "common_type rule requires 'param' key" }
        ...
        when "array"
          if spec.key?("element_type")
            ...
            raise Bug, "array rule requires either 'element_type' or 'element_type_param' key"
          end
        ...
        when "tuple"
          if spec.key?("element_types")
            ...
            raise Bug, "tuple rule requires either 'element_types' or 'element_types_param' key"
          end

        when "scalar"
          kind = spec.fetch("kind") { raise Bug, "scalar rule requires 'kind' key" }
          kind_sym = kind.to_sym
          raise Bug, "scalar rule has unknown kind: #{kind}" unless Kumi::Core::Types::Validator.valid_kind?(kind_sym)
