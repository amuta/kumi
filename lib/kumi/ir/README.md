...
- **Loop** – (Removed for now.) The structured-loop layer is being redesigned to
  sit *after* VecIR/BufIR once those stages are stable.
...
materialized during analysis (`LowerToDFIRPass`). LoopIR previously mirrored
those graphs but has been removed while we rebuild the lower pipeline via
VecIR/BufIR.
...
Pretty printer commands are available for DFIR layers:
